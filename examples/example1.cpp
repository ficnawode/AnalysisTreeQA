...
void example(const std::string& filelist) {
  ...
  for (int j = 0; j < 3; j++) {
    ...
    auto bdt_var_name = "VtxTracks_pred.onnx_pred_0_" + j_str;
  ...
  task->AddH1({"momentum p, []", Variable::FromString("VtxTracks_pred.p"), {200, -6, 6}});
  task->AddH1({"pid, []", Variable::FromString("VtxTracks_pred.mc_pdg"), {200, -6, 6}});

  Variable p_over_q = Variable("pq", {{"VtxTracks_pred", "p"}, {"VtxTracks_pred", "q"}}, [](std::vector<double>& var) { return var.at(0) / var.at(1); });
  auto mass2 = Variable::FromString("TofHits.mass2");

  mass2.SetName("m2");
  task->AddH2({"p/q, (GeV/c)", p_over_q, {400, -12, 12}}, {"m^{2}, (GeV^{2}/c^{4})", mass2, {400, -1, 2}});

  auto q_cut = RangeCut("VtxTracks_pred.q", 0, 2);

  // PROTONS
  {
    auto bdt_cut = RangeCut("VtxTracks_pred.onnx_pred_0_0", 0.999, 1.1);
    int pdg = 2212;
    auto simpdg_cut = EqualsCut("VtxTracks_pred.mc_pdg", pdg);
    auto wrong_pid_cut = CreateNotEqualsCut("VtxTracks_pred.mc_pdg", pdg);
  ...
  // kaons
  {
    auto bdt_cut = RangeCut("VtxTracks_pred.onnx_pred_0_1", 0.999, 1.1);
    int pdg = 321;
    auto simpdg_cut = EqualsCut("VtxTracks_pred.mc_pdg", pdg);
    auto wrong_pid_cut = CreateNotEqualsCut("VtxTracks_pred.mc_pdg", pdg);
  ...
  // PROTONS
  {
    auto bdt_cut = RangeCut("VtxTracks_pred.onnx_pred_0_2", 0.999, 1.1);
    std::vector<double> pdg{-13, 211, -11};
    auto simpdg_cut = CreateEqualsVectorCut("VtxTracks_pred.mc_pdg", pdg);
    auto wrong_pid_cut = CreateNotEqualsVectorCut("VtxTracks_pred.mc_pdg", pdg);
