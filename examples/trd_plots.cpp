...
void pid_gauss(const std::string filelist) {
  ...
  task->SetOutputFileName("trd_plots.root");
  ...
  task->AddH2({"TRD Energy Loss 0, (GeV)", {"TrdTracks", "energy_loss_0"}, {100, -1, 12}}, {"m^{2}, (GeV^{2}/c^{4})", mass2, {400, -1, 2}});

  {
    int pdg = 2212;
    auto simpdg_cut = EqualsCut("VtxTracks.mc_pdg", pdg);
    auto true_pdg_cut = new AnalysisTree::Cuts("P_pdg", {simpdg_cut});
    task->AddH2({"TRD Energy Loss 0, (GeV)", {"TrdTracks", "energy_loss_0"}, {100, -1, 12}}, {"m^{2}, (GeV^{2}/c^{4})", mass2, {400, -1, 2}}, true_pdg_cut);
  }
